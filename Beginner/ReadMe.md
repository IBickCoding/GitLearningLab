## Table of Contents

> To navigate, simply click on the title or topic below. Each document is in the order in which they should be read.

### [Introduction to Version Control and Git](MdFileVersions/IntroductionToVersionControlAndGit.md)

- [Version Control vs Source Control](MdFileVersions/IntroductionToVersionControlAndGit.md#version-control-vs-source-control)
- [Why Git Matters](MdFileVersions/IntroductionToVersionControlAndGit.md#why-git-matters)
- [Why Use Git](MdFileVersions/IntroductionToVersionControlAndGit.md#why-use-git)
- [Benefits of Using Git](MdFileVersions/IntroductionToVersionControlAndGit.md#benefits-for-individuals)
- [Basic Git Terminology](MdFileVersions/IntroductionToVersionControlAndGit.md#basic-git-terminology-to-know)
- [The Git Workflow Mental Model](MdFileVersions/IntroductionToVersionControlAndGit.md#the-git-workflow-mental-model)

### [Installing Git](MdFileVersions/InstallingGit.md)

- [Installing Git on Windows](MdFileVersions/InstallingGit.md#windows-installation)
- [Installing Git on Linux](MdFileVersions/InstallingGit.md#linux-installation)
- [Installing Git on MacOS](MdFileVersions/InstallingGit.md#macos-installation)
- [Verifying Version and Installation](MdFileVersions/InstallingGit.md#verification-of-installation)
- [Configuring Your Git Username and Email Address](MdFileVersions/InstallingGit.md#configuring-your-git-username-and-email-address)

### [Understanding Repositories](MdFileVersions/UnderstandingRepositories.md)

- [What is a Repository?](MdFileVersions/UnderstandingRepositories.md#what-is-a-repository)
- [Understanding Local Repositories](MdFileVersions/UnderstandingRepositories.md#understanding-local-repositories)
- [Understanding Remote Repositories](MdFileVersions/UnderstandingRepositories.md#understanding-remote-repositories)

### [Creating Your First Repository](MdFileVersions/CreatingYourFirstRepository.md)

- [Creating a Local Repository](MdFileVersions/CreatingYourFirstRepository.md#creating-a-local-repository)
- [The .git Folder](MdFileVersions/CreatingYourFirstRepository.md#the-git-folder)
- [Viewing Hidden Files](MdFileVersions/CreatingYourFirstRepository.md#viewing-hidden-files)
- [Hands-On Guided Activity](MdFileVersions/CreatingYourFirstRepository.md#hands-on-guided-activity)

### [Making Commits and Tracking Changes](MdFileVersions/MakingCommitsAndTrackingChanges.md)

- [The Working Directory](MdFileVersions/MakingCommitsAndTrackingChanges.md#the-working-directory)
- [The Staging Area](MdFileVersions/MakingCommitsAndTrackingChanges.md#the-staging-area)
- [Commits](MdFileVersions/MakingCommitsAndTrackingChanges.md#commits)
- [Hands-On Guided Activity](MdFileVersions/MakingCommitsAndTrackingChanges.md#hands-on-guided-exercise)
- [How Git Tracks Changes Over Time](MdFileVersions/MakingCommitsAndTrackingChanges.md#how-git-tracks-changes-over-time)

### [Remote Repositories](MdFileVersions/RemoteRepositories.md)

- [Creating a GitHub Account](MdFileVersions/RemoteRepositories.md#creating-a-github-account)
- [Creating a Remote Repository and Linking a Local Repository](MdFileVersions/RemoteRepositories.md#creating-a-remote-repository-and-linking-the-local-repository)
- [Verify Link Between Remote and Local Repositories](MdFileVersions/RemoteRepositories.md#verify-link-between-local-and-remote-repository)
- [Difference Between Local and Remote Repositories](MdFileVersions/RemoteRepositories.md#difference-between-local-and-remote-repository)
- [Hands-On Example](MdFileVersions/RemoteRepositories.md#hands-on-example)
- [Cloning a Remote Repository](MdFileVersions/RemoteRepositories.md#cloning-a-repository)

### [Git Cheat Sheet](MdFileVersions/GitCheatSheet.md)

- Command Line Control of Git

### [Beginner Git Learning Lab Assignment](MdFileVersions/BeginnerGitLearningLabAssignment.md)

- This assignment covers creating a local and remote repository, linking them, making adding/committing/pushing changes, and a knowledge check.

### [Git Beginner Pre/Post Assessment](MdFileVersions/GitBeginnerPrePostAssessment.md)

- This assessment is designed to be used both before learners go through the course and after to measure comprehension.

### [Git Fundamentals Kahoot](https://create.kahoot.it/share/git-version-control-fundamentals/9d15fa52-1ba3-4f37-b3e7-dc549be3c4fe)

- (To be used for pre/post assessment by educators to determine comprehension of concepts covered.)